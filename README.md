...
| @google/genai | 1.43 | Gemini AI SDK (contract scanner + config builder) |
...
│   │   │   ├── BatchIngestion.tsx     # Config Review — review/edit/approve pending contract types
...
│   │   │   ├── batch.ts               # Pending contract type review (approve/reject/edit)
...
│   │   │
│   │   ├── middleware/
│   │   │   ├── auth.ts                # requireAuth, requireAdmin, requireSuperAdmin, requireGod + suspension check
...
│   │   │   ├── rules.ts              # Zod schemas for typed Rules and extracted ContractType
│   │   │   └── locals.ts              # Zod schemas for LocalConfig create/update
│   │   │
│   │   ├── scripts/
│   │   │   └── config-builder.ts      # CLI tool — wage agreement PDF → PendingContractType
│   │   │
│   │   └── utils/
...
Upload a photo of a physical union contract document. **Google Gemini 3.1 Flash Lite** analyzes the image server-side and extracts all fields, wage scales, rules, and legal text into a structured `ContractType` JSON object using a strict response schema. The output can be copied and pasted into the LocalConfig Editor.
